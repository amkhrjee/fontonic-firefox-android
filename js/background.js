browser.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.action === "on-page-load") {
        ...
        */
        ...
        browser.storage.sync.get(["global"]).then((is_global) => {
            browser.storage.sync.get(["override"]).then((is_override) => {
                browser.storage.sync
                    ...
                    if ("global" in is_global && is_global["global"]) {
                        if ("exempts" in exempts_list &&
                            ...
                            browser.storage.sync.get([message.domain]).then((is_domain) => {
                        ...
                        else if ("override" in is_override && is_override["override"]) {
                            // global & override both are on so apply global fonts to all sites
                            console.log("GLOBAL IS ON + OVERRIDE IS ON");
                            browser.storage.sync.get(["global_fonts"]).then((result) => {
